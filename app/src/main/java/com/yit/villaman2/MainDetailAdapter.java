...
//		//////////////////////////////
		...
		if(isWebView == true) {
			...
			if (strGubun.equals("main")) { // 구옥빌라, 신축빌라

				holder.webview_list.loadUrl("http://yes-vil.kr/w/vila_app_list.php"
												+ "?main_id=" + adapterList.get(position).getID()
												+ "&usr_id=" + Util.getPhoneNumber(activity)
												);
			} else {
				holder.webview_list.loadUrl("http://yes-vil.kr/w/vila_app_list_n.php"
						+ "?main_id=" + adapterList.get(position).getID()
						+ "&usr_id=" + Util.getPhoneNumber(activity)
				);
			}
		...
		holder.btnGoMap.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				/////////////////////////////////////////////////////////////
				// 헨드폰번호가 없거나, 회원이 아닐경우는 roadView 사용불가
				if (Util.getPhoneNumber(activity).equals("00000000000") || adapterList.get(position).getMa_bunji2().equals("***"))
				{
					return;
				}
				/////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		// 즐겨찾기 추가/삭제
		holder.btnGoFav.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {

				if (v.getTag().equals("ico_fav_add")){	//즐겨찾기 추가
					// Thread로 웹서버에 접속
					new Thread() {
						public void run() {
							String requestURL = Util.getURL_IT() +"/app/saveFavApp.do?CRUD=C&usr_id=" + Util.getPhoneNumber(activity) +"&ma_id="+adapterList.get(position).getID(); // TODO
							String result = openHttpURLConnection(requestURL);
							Bundle bun = new Bundle();
							bun.putString("KEY", result);
					...
					v.setBackgroundResource(R.drawable.ico_fav_view);
					v.setTag("ico_fav_view");
					Toast.makeText(activity, "즐겨찾기 목록에 추가 되었습니다."   , Toast.LENGTH_SHORT).show();

				} else{	//즐겨찾기 삭제
					// Thread로 웹서버에 접속
					new Thread() {
						public void run() {
							String requestURL =Util.getURL_IT() +"/app/saveFavApp.do?CRUD=D&usr_id=" + Util.getPhoneNumber(activity) +"&ma_id="+adapterList.get(position).getID(); // TODO
							String result = openHttpURLConnection(requestURL);
							Bundle bun = new Bundle();
							bun.putString("KEY", result);
							Message msg = handler.obtainMessage();
					...
					v.setBackgroundResource(R.drawable.ico_fav_add);
					v.setTag("ico_fav_add");
					Toast.makeText(activity, "즐겨찾기 목록에서 삭제 되었습니다."   , Toast.LENGTH_SHORT).show();
				}
			}
		});
////////////////////////////////////////////////////////////////////////////////////////////////////
