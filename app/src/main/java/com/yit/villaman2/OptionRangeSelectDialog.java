...
import com.yit.villaman2.RangeSeekBar.OnRangeSeekBarChangeListener;
...
public class OptionRangeSelectDialog extends BaseDialog {
	...
	// private Button btnRangeClear;
	private TextView tvMaeCheckBox;
	private LinearLayout inputSeek;
	...
	private void startView_onCreate() {
		...
		inputSeek = (LinearLayout) findViewById(R.id.inputSeek);
		tvMaeCheckBox = (TextView) findViewById(R.id.tvMaeCheckBox);
		...
		RangeSeekBar<Integer> inputSeekBar = new RangeSeekBar<Integer>(1, 30, context);
		inputSeek.addView(inputSeekBar);
		inputSeekBar.setNotifyWhileDragging(true);
		
		if(FProPrefer.with(context).getMinInput() != 0)
			inputSeekBar.setSelectedMinValue(FProPrefer.with(context).getMinInput());
		else
			FProPrefer.with(context).setMinInput(1);
			
		if(FProPrefer.with(context).getMaxInput()  != 0)
			inputSeekBar.setSelectedMaxValue(FProPrefer.with(context).getMaxInput());
		else
			FProPrefer.with(context).setMaxInput(30);
		
		tvMaeCheckBox.setText("▣ 매매가 : " + (FProPrefer.with(context).getMinInput() * 1000) + " ~ " + (FProPrefer.with(context).getMaxInput() * 1000));
		
		inputSeekBar.setOnRangeSeekBarChangeListener(new OnRangeSeekBarChangeListener<Integer>() {

			@Override
			public void onRangeSeekBarValuesChanged(RangeSeekBar<?> bar, Integer minValue, Integer maxValue) {
				tvMaeCheckBox.setText("▣ 매매가 : " + (minValue * 1000) + " ~ " + (maxValue * 1000));
				maxInput = maxValue * 1000;
				FProPrefer.with(context).setMinInput(minValue);
				FProPrefer.with(context).setMaxInput(maxValue);
			}
		});
	...
	private void startAction_onCreate() {
		...
		btnRangeConfirm.setOnClickListener(new android.view.View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				detailMap.put("ma_mae_ney1", String.valueOf(FProPrefer.with(context).getMinInput()*1000));
				detailMap.put("ma_mae_ney2", String.valueOf(FProPrefer.with(context).getMaxInput()*1000));
		...
		btnRangeClear.setOnClickListener(new android.view.View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				FProPrefer.with(activity).setMinInput(0);
				FProPrefer.with(activity).setMaxInput(0);
